...
class LightGun:
    ...
    def getPosition(self, frame, frame_center, highest_scoring_obj):
        ...
        filtered_coords = smoothed_coords[-1] * [2.0, -2.0] + [-1.0, 1.0]
...
def main():
    ...
    parser.add_argument('-v2', '--videodevice2', type=int, default=None, help='Second video device index (optional)')
    ...
    if args.videodevice2 is not None:
        light_gun2 = LightGun(videodevice=args.videodevice2, width=args.width, height=args.height, filter_len=args.filter_len)
    ...
    while True:
        ...
        if tv_objs:
            ...
            if any(x**2 > 1.0 for x in filtered_coords):
                gamepad.press_button(button=vg.XUSB_BUTTON.XUSB_GAMEPAD_Y)
                gamepad.update()
            else:
                gamepad.release_button(button=vg.XUSB_BUTTON.XUSB_GAMEPAD_Y)
                gamepad.left_joystick_float(x_value_float=filtered_coords[0], y_value_float=filtered_coords[1])
                gamepad.update()

        if args.videodevice2 is not None and frame2 is not None:
            ...
            if args.debug:
                cv2.drawMarker(frame2, frame_center2, (255, 0, 0), cv2.MARKER_CROSS, 20, 2) # Mark centre of frame

            if tv_objs2:
                highest_scoring_obj2 = max(tv_objs2, key=lambda tv: tv.score)
                filtered_coords2 = light_gun2.getPosition(frame2, frame_center2, highest_scoring_obj2)
                ...
                if args.debug:
                    ...
                    cv2.putText(frame2, f"({filtered_coords2[0]:.2f}, {filtered_coords2[1]:.2f})",
                                (10, frame2.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                gamepad.right_joystick_float(x_value_float=filtered_coords2[0], y_value_float=filtered_coords2[1])
                gamepad.update()
